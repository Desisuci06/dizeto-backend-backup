...
func (hc *ClientController) CreateClient(c *gin.Context) {
	...
	if err := hc.clientService.CreateClient(clientDTO.Title, clientDTO.Image); err != nil {
...
func (hc *ClientController) UpdateClient(c *gin.Context) {
	...
	if err := hc.clientService.UpdateClient(id, clientDTO.Title, clientDTO.Image); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
